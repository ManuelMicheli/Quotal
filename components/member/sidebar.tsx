 ...
 * top bar. On desktop it replaces both, keeping the primary canvas free
 * for content so cards can spread horizontally instead of stacking in
 * the centre.
 */
import {
  CreditCardIcon,
...
export function MemberSidebar({
  ...
  return (
    <aside
      ...
      className="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col border-r border-border/60 bg-card/60 backdrop-blur-xl supports-[backdrop-filter]:bg-card/45 lg:flex"
          ...
          {TABS.map((tab) => {
            ...
            return (
              <li key={tab.href}>
                ...
                >
                  <Icon size={18} aria-hidden="true" strokeWidth={active ? 2.25 : 1.75} />
                  {tab.label}
                </Link>
              </li>
            )
